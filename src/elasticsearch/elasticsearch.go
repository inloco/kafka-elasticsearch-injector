...
func (d recordDatabase) Insert(records []*models.ElasticRecord) (*InsertResponse, error) {
	...
	for _, rec := range records {
		reqBody, _ := json.Marshal(rec.Json["body"])
		gzipWrite(&buf, []byte(reqBody))
		contentLength := fmt.Sprint(buf.Len())
...
func NewDatabase(logger log.Logger, metrics metrics.MetricsPublisher) RecordDatabase {
